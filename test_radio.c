...
#define N_BENCHMARK_ITERATIONS (2048)
#define TIMER_FREQUENCY (125e6)
...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    for (int i=0;i<N_BENCHMARK_ITERATIONS;i++)
    ...
    unsigned int bytes_transferred = N_BENCHMARK_ITERATIONS * 4; // 4 bytes per transfer
    float time_spent =  (stop_time - start_time) / TIMER_FREQUENCY; // change obviously
    float throughput = (float) bytes_transferred / time_spent / (1 << 20); // 2^20 B = 1 MB
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Jonathan Piland - Custom Peripheral Demonstration\n\r");
